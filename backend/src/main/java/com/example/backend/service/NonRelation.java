...
public class NonRelation {
    ...
    @Transactional
    public void testChildRelational(ParentRequest parentRequest,
                                    ChildRequest childRequest){
        Parent parent = new Parent(parentRequest.getEmail(), parentRequest.getName());
        parentRepository.save(parent);

        Child child = new Child(childRequest.getLoginID(),childRequest.getPassword(),
    ...
    @Transactional
    public void relationmethod(ParentRequest parentRequest,
                               ChildRequest childRequest){
        Parent parent = new Parent(parentRequest.getEmail(), parentRequest.getName());
        parentRepository.save(parent);
